...
import { TransactionDB } from 'src/entities/transaction';
...
const url_postgres = 'http://202.44.231.125:1752/device_data/';
const url_postgres_v3 = 'http://202.44.231.125:9109/device_data/';
...
export class TransactionService implements OnApplicationBootstrap {
    ...
    async create(createTransactionDto: CreateTransactionDto) {
        ...
        try {
            ...
            // transactions.serialNumber = createTransactionDto.serialNumber ? createTransactionDto.serialNumber : null;
            ...
            // serialNumber
            reNewTransactionEa['serialNumber'] = createTransactionDto.serial_number ? createTransactionDto.serial_number : null;

            reNewTransactionEa['date_data7'] = moment().tz('asia/Bangkok').format('YYYY-MM-DD HH:mm:ss');

            await axios
                .post(url_postgres, reNewTransactionEa)
                .then((results) => {
                    console.log('Result device =:} ', results.data);
                })
                .catch((error) => {
                    console.log('Failed to fetch -> ', error);
                });
            await axios
                .post(url_postgres_v3, reNewTransactionEa)
                .then((results) => {
                    console.log('Result device =:} ', results.data);
                })
                .catch((error) => {
